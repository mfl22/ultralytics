...
def write_results_score(filename, results):
    """
    Save tracking result for later interpolation.

    Inputs:
        filename - file path to save to (.txt file)
        results - tracking results; format:
            (frame_id, track_id, x1, y1, w, h, score)
            (score optional)
...
def dti(txt_path, save_path, n_min=25, n_dti=20):
    ...
    Inputs:
        txt_path - .tyt file with saved results
            (see write_results_score() function above)
        save_path - path to save the interpolated tracks to
        kwargs:
            n_min - minimum length of tracklet to consider it (25)
            n_dti - maximum distance for connecting disconnected tracklets
                (20)
    """
            ...
            if n_frame > n_min:
                ...
                if num_dti > 0:
                    ...
                    for n in range(num_dti):
                        ...
                        data_dti[n, 2:6] = frames_dti[
                            list(frames_dti.keys())[n]
                        ]
...
if __name__ == '__main__':
    ...
    txt_path = ('/opt/tiger/demo/ByteTrack/YOLOX_outputs/'
                'yolox_x_mix_det/track_results')
    save_path = ('/opt/tiger/demo/ByteTrack/YOLOX_outputs/'
                 'yolox_x_mix_det/track_results_dti')
